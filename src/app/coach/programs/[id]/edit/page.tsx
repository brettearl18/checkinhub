...
interface ProgramResponse {
  ...
  phaseName?: string;
  weeks: { days: { name?: string; blocks: { type: string; restSeconds?: number; exercises: { exerciseId: string; sets?: string; reps?: string; notes?: string }[] }[] }[] }[];
}
...
function blockLabel(type: BlockType) {
  return type === "straight_sets" ? "Straight sets" : type === "superset" ? "Superset" : "Circuit";
}

export default function CoachEditProgramPage() {
  ...
  useEffect(() => {
    ...
    setAuthError(false);
    Promise.all([
      fetchWithAuth(`/api/coach/programs/${id}`),
      fetchWithAuth("/api/coach/exercises"),
    ])
      .then(([progRes, exRes]) => {
        if (cancelled) return [null, []] as [ProgramResponse | null, { id: string; name: string }[]];
        if (progRes.status === 401 || exRes.status === 401) {
          setAuthError(true);
          return [null, []] as [ProgramResponse | null, { id: string; name: string }[]];
        }
        return Promise.all([progRes.ok ? progRes.json() : null, exRes.ok ? exRes.json() : []]);
      })
      .then(([prog, exercises]: [ProgramResponse | null, { id: string; name: string }[]]) => {
        if (!Array.isArray(exercises)) return;
        if (cancelled || !prog) return;
        const map: Record<string, string> = {};
        for (const e of Array.isArray(exercises) ? exercises : []) {
          if (e && e.id && e.name) map[e.id] = e.name;
        }
        setName(prog.name ?? "");
        setDescription(prog.description ?? "");
        setDurationWeeks(prog.durationWeeks ?? "");
        setPhaseName(prog.phaseName ?? "");
        const w = prog.weeks ?? [];
        setWeeks(
          w.length
            ? w.map((week) => ({
                days: (week.days ?? []).map((d, di) => ({
                  name: d.name?.trim() || `Day ${di + 1}`,
                  blocks: (d.blocks ?? []).map((b) => ({
                    type: (b.type === "superset" || b.type === "circuit" ? b.type : "straight_sets") as BlockType,
                    restSeconds: typeof b.restSeconds === "number" ? b.restSeconds : "",
                    exercises: (b.exercises ?? []).map((ex) => ({
                      exerciseId: ex.exerciseId ?? "",
                      exerciseName: map[ex.exerciseId ?? ""] ?? "Exercise",
                      sets: ex.sets ?? "",
                      reps: ex.reps ?? "",
                      notes: ex.notes ?? "",
                    })),
  ...
  const addExerciseToBlock = (weekIdx: number, dayIdx: number, blockIdx: number, exercise: { id: string; name: string }) => {
    setWeeks((w) => {
      const next = [...w];
      const block = next[weekIdx].days[dayIdx].blocks[blockIdx];
      const alreadyEmpty = block.exercises.some((e) => e.exerciseId === exercise.id && !e.sets.trim() && !e.reps.trim() && !e.notes.trim());
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetchWithAuth(`/api/coach/programs/${id}`, {
        method: "PATCH",
  ...
  if (!id) {
    return (
      <div className="space-y-6">
        <Link href="/coach/programs" className="text-sm text-[var(--color-primary)] hover:underline">← Programs</Link>
        <p className="text-[var(--color-error)]">Invalid program.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <Link href="/coach/programs" className="text-sm text-[var(--color-primary)] hover:underline">
          ← Programs
        </Link>
        <h1 className="mt-1 text-2xl font-semibold text-[var(--color-text)]">Edit program</h1>
        <p className="mt-1 text-sm text-[var(--color-text-secondary)]">
          Update weeks, days, and blocks (straight sets or supersets). Set sets×reps and rest between exercises.
        </p>
      </div>
        ...
        <Card className="p-6 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              required
              className="w-full max-w-md rounded-md border border-[var(--color-border)] bg-[var(--color-bg)] px-3 py-2 text-sm text-[var(--color-text)]"
          ...
          <div className="flex flex-wrap gap-4">
            <div>
              ...
              <input
                ...
                className="w-24 rounded-md border border-[var(--color-border)] bg-[var(--color-bg)] px-3 py-2 text-sm text-[var(--color-text)]"
                placeholder="Optional"
              />
            </div>
            ...
            <div key={weekIdx} className="rounded-lg border border-[var(--color-border)] bg-[var(--color-bg)] p-4 space-y-4">
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <Button type="button" variant="ghost" className="text-sm py-1.5 min-h-0" onClick={() => addDay(weekIdx)}>
                    Add day
                  </Button>
                  {weeks.length > 1 && (
                    <Button type="button" variant="ghost" className="text-sm py-1.5 min-h-0 text-[var(--color-error)]" onClick={() => removeWeek(weekIdx)}>
                      Remove week
                    </Button>
                  )}
                </div>
              ...
              {week.days.length === 0 && (
                <p className="text-sm text-[var(--color-text-muted)]">No days yet. Add a day, then add blocks (straight sets or supersets).</p>
              )}
                ...
                <div key={dayIdx} className="rounded border border-[var(--color-border)] bg-[var(--color-bg-elevated)] p-3 space-y-3">
                  <div className="flex flex-wrap items-center gap-2">
                    <input
                      type="text"
                      value={day.name}
                      onChange={(e) => setDayName(weekIdx, dayIdx, e.target.value)}
                      className="flex-1 rounded-md border border-[var(--color-border)] bg-[var(--color-bg)] px-2 py-1.5 text-sm font-medium text-[var(--color-text)]"
                      placeholder="Day name"
                    />
                    {week.days.length > 1 && (
                      <Button type="button" variant="ghost" className="text-sm py-1.5 min-h-0 text-[var(--color-error)]" onClick={() => removeDay(weekIdx, dayIdx)}>
                        Remove day
                      </Button>
                    )}
                  </div>
                    ...
                    <div key={blockIdx} className="rounded-lg border border-[var(--color-border)] bg-[var(--color-bg)] p-3 space-y-2">
                      ...
                      {block.exercises.length === 0 || (block.type !== "straight_sets" && (block.type === "superset" ? block.exercises.length < 2 : block.exercises.length < 3)) ? (
                        <Button type="button" variant="ghost" className="text-sm py-1.5 min-h-0" onClick={() => openPicker(weekIdx, dayIdx, blockIdx)}>
                          {block.type === "straight_sets" ? "Pick exercise" : `Add exercise${block.exercises.length > 0 ? "" : ` (2+ for superset, 3+ for circuit)`}`}
                        </Button>
